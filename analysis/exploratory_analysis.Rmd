...
The analytical plan will involve continuing to explore trends between weather data and weather-related flight delays. I believe other factors such as geographic location and time ultimately affect the weather conditions of the departure airport for a particular flight.

I used a subset sample of 10000 flights with equal balance of weather delayed and other flights. Pre-processing techniques was used to ensure the data is of good quality and format before modelling. To predict flight delays, I started with a decision tree model, and evaluate performance. I improved on this by using ensemble techniques such as random forests, bagging, adaboost, and xgboost. Performances between each model are evaluated using 10-fold cross-validation, and appropriate criteria such as P-value and accuracy. This is the preliminary modelling, and is used to find the most important features.

\pagebreak
## 6. Discussion

For the initial model, I used both the numerical and categorical weather data. Unsurprisingly, I found that the decision tree model performed the worst, with an accuracy of 61%. I then improved this by using Random Forests, which improved the test accuracy to 63%. Adaboost didn't produce particularly great results, with an accuracy of 62%, while XGBoost had the highest accuracy of 64.6%. In the end, XGBoost seemed to be the best model for this particular dataset. Note that I used 10-fold cross-validation to conduct hyper parameter tuning for each model, and final accuracy is based on the best model's performance on the test data. I explored a reasonable range of hyper parameters such as max tree depth and learning rate, where there is a trade-off on search comprehensiveness and optimisation bias.

I repeated the above modeling steps, but with only using categorical weather data. The accuracy for XGBoost dropped to 59.8%. I repeated the modeling using XGBoost with only numerical weather data and found an accuracy score of 64.6%, which is the same score as the model with combined weather data. I can infer from this that categorical weather data, such as "heavy rain" or "light snow," doesn't provide much more information to the model, as this is already represented in primary weather data. Additionally, categorical weather data is often quite inconsistent, as I have observed that some observations might indicate rainfall in cm for the hour, while the categorical data would indicate that it was not raining. After some research, I removed the dew point variable from the dataset, as this is mathematically related to temperature and humidity. This resulted in an increase in accuracy to 65%.
